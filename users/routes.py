...
def read_read_all(page: Optional[int] = 0, current_user: UserModel = Depends(get_current_user), db: Session = Depends(get_db)):
    users, total = get_all_users(db, current_user, page=page)
